...
class image(db.Model):
    id: Mapped[int] = mapped_column(primary_key=True)
    image: Mapped[str] = mapped_column(unique=False, nullable=False)
...
def uploud():
    ...
    if not picture:
        flash('no pics were uploaded')
        return  redirect(url_for("uploud")) 

    file_name = secure_filename(picture.file_name)
    mimetype = picture.mimetype

    img = img(image=picture.read(), mimetype=mimetype, name=file_name)
    db.session.add(img)
    db.session.commit()
